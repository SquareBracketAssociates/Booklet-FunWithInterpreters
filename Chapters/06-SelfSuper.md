...
with a specific focus on the `self`' and `super` peudo variables. 
Pseudo variables in Pharo are variables that are set by the system during its execution.
...
Our new test is still failing, but we are now ready to make it pass.
...
```
CInterpreter >> visitVariableNode: aNode
	aNode isSelfSend | aNode isSuperSend
		ifTrue: [ ^ self receiver ]
```
